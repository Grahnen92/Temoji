    ...
    public class INfiniDyGrassField : MonoBehaviour
    {
        ...
        public void Start()
        {
            ...
            if (Application.isPlaying)
            {
                ...
                //v1.1
                if (MoveWithObject && PaintedOnOBJ != null && Grow_tree)
                {//v1.1a
                 //ParentToObject = true;
        ...
        void StartP()
        {
            ...
            else
            {
                ...
                if (this.transform.localScale != Vector3.one && this.transform.parent.name == "Grass Holder")
                {//v1.4
        ...
        public void Update()
        {
            ...
            if (Interactive_tree | (!Interactive_tree && !growth_over && !Grow_tree_ended))
            {
                ...
                if ((Grow_in_Editor & !Application.isPlaying & (!Has_Run_once | (Has_Run_once & root_tree == null))))
                {
                    ...
                    //v1.1a
                    if (!Grow_tree & !Has_Run_once)
                    { //v1.5 - fix scaling issue when painting in editor
                        if (Registered_Brances[0].localScale.x < End_scale)
                        {
                            for (int i = 0; i < Registered_Brances.Count; i++)
                            {
            ...
            if (!removed_shadows & no_shadows_on_dynamic)
            {
                ...
                if (Rm_shad_above_lvl < 0)
                {
                    ...
                    for (int i = 0; i < filters.Length; i++)
                    {
                        ...
                        {
                            if (curRenderer.sharedMaterial.name.Contains("LOD"))
                            {//LOD
                                if (Rm_cast)
                                {
